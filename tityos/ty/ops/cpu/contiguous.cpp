...
namespace ty {
namespace internal {
    template <typename T> void contiguousCpuKernel(const TensorView& outView, const TensorView& tensorView) {
        ...
        const T* __restrict__ tensorData = static_cast<T*>(tensorView.data) + tensorView.offset;
    ...
    BaseTensor backend::CPUBackend::contiguous(const BaseTensor& tensor) {
        ...
        DISPATCH_KERNEL_DTYPE_TABLE(kernelTable, contiguousCpuKernel, (const TensorView&, const TensorView&))
