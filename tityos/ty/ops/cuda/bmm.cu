...
namespace internal {
    ...
    template <typename T>
    __global__ void
    bmmCudaKernel(int batches, int M, int N, int K,
                  int sx1, int sy1, int sz1,
                  int sx2, int sy2, int sz2,
                  int sxRes, int syRes, int szRes,
        ...
        if (x < M && y < N && batch < batches) {
            ...
            // C = α*(A@B)+β*C
            resultTensorView.at3d(x, y, batch, sxRes, syRes, szRes) = alpha * tmp + beta * resultTensorView.at3d(x, y, batch, sxRes, syRes, szRes);
        };
    };
    ...
    void launchBMMKernel(const BaseTensor& batch1, const BaseTensor& batch2,
        ...
        bmmCudaKernel<T><<<grid, block>>>(batches,
            rows, cols, tensorView1.shape[2],
            tensorView1.strides[2], tensorView1.strides[1], tensorView1.strides[0],
            tensorView2.strides[2], tensorView2.strides[1], tensorView2.strides[0],
            resultTensorView.strides[2], resultTensorView.strides[1], resultTensorView.strides[0],
