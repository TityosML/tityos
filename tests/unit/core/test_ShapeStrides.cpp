...
TEST_CASE("ShapeStrides Slicing", "[ShapeStrides]") {
    ...
    CHECK(example2D.slice(0, 0, 2) == ty::internal::ShapeStrides({2, 4}, {4, 1}, 0, 2));
    CHECK(example2D.slice(0, 0, 2).slice(1, 1, 3) == ty::internal::ShapeStrides({2, 2}, {4, 1}, 1, 2));
    CHECK(example2D.slice(0).slice(1, -2) == ty::internal::ShapeStrides({3, 2}, {4, 1}, 2, 2));
    CHECK(example2D.slice(0, 0, 1).slice(1, 1) == ty::internal::ShapeStrides({1, 3}, {4, 1}, 1, 2));
