...
namespace internal {
    template <typename T>
    void addAvxKernel(const TensorView& outView, const TensorView& tensor1View, const TensorView& tensor2View) {
        using Vec = typename AvxTraits<T>::Vec;
        constexpr int lanes = AvxTraits<T>::lanes;

        const size_t total = std::accumulate(outView.shape, outView.shape + outView.ndim, 1, std::multiplies<size_t>());

        T* outData = static_cast<T*>(outView.data) + outView.offset;
        const T* tensor1Data = static_cast<T*>(tensor1View.data) + tensor1View.offset;
        const T* tensor2Data = static_cast<T*>(tensor2View.data) + tensor2View.offset;
    ...
    void addAvx(BaseTensor& result, const BaseTensor& tensor1, const BaseTensor& tensor2) {
        DISPATCH_KERNEL_DTYPE_TABLE(kernelTable, addAvxKernel,
