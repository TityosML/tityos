...
TEST_CASE("Benchmark CUDA Tensor Add Broadcasting (large tensors)", "[benchmark][broadcast][add]") {
    ...
    ty::Tensor A(dataA, std::vector<size_t>({D0, D1, D2}), {ty::DeviceType::CUDA, 0}, ty::DType::Int32);
    ...
    ty::Tensor B(dataB, std::vector<size_t>({E0, E1, E2}), {ty::DeviceType::CUDA, 0}, ty::DType::Int32);
...
TEST_CASE("Benchmark Batched MatMul CUDA (large tensors)", "[benchmark][bmm][matmul]") {
    ...
    ty::Tensor A(dataA, std::vector<size_t>{BATCH, M, N}, {ty::DeviceType::CUDA, 0}, ty::DType::Int32);
    ...
    ty::Tensor B(dataB, std::vector<size_t>{BATCH, N, K}, {ty::DeviceType::CUDA, 0}, ty::DType::Int32);
