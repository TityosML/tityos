...
Tensor add(const Tensor& tensor1, const Tensor& tensor2) {
    auto broadcastShape =
        internal::broadcastShape(tensor1.getShape(), tensor1.getNDim(), tensor2.getShape(), tensor2.getNDim());
    ...
    auto resultBase = std::make_shared<internal::BaseTensor>(
        b->add(*broadcastedTensor1.getBaseTensor(), *broadcastedTensor2.getBaseTensor()));

    auto result = Tensor(resultBase);

    auto contextStorage = internal::GradientContextStorage(tensor1, tensor2);
    auto context = internal::GradientContext(contextStorage, [](const internal::GradientContextStorage& tensors) {
