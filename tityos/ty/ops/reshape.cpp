...
namespace ty {
namespace internal {
    BaseTensor reshape(const BaseTensor& tensor, const TensorShape& newShape, size_t ndim) {
    ...
    BaseTensor view(const BaseTensor& tensor, const TensorShape& newShape, size_t ndim) {
...
Tensor reshape(const Tensor& tensor, const TensorShape& newShape, size_t ndim) {
    return Tensor(std::make_shared<internal::BaseTensor>(internal::reshape(*tensor.getBaseTensor(), newShape, ndim)));
}
...
Tensor view(const Tensor& tensor, const TensorShape& newShape, size_t ndim) {
    ...
    return Tensor(std::make_shared<internal::BaseTensor>(internal::view(*tensor.getBaseTensor(), newShape, ndim)));
}
} // namespace ty
