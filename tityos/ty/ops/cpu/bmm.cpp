...
namespace internal {
    template <typename T>
    void bmmCpuKernel(const TensorView& outView, const TensorView& tensor1View, const TensorView& tensor2View) {
...
#pragma omp parallel for
        for (size_t batch = 0; batch < outView.shape[0]; batch++) {
            for (size_t m = 0; m < tensor1View.shape[1]; m++) {
                for (size_t k = 0; k < tensor2View.shape[2]; k++) {
                    size_t outIdx = batch * outView.strides[0] + m * outView.strides[1] + k * outView.strides[2];

                    outData[outIdx] = 0;
                    for (size_t n = 0; n < tensor2View.shape[1]; n++) {
                        size_t tensor1Idx =
                            batch * tensor1View.strides[0] + m * tensor1View.strides[1] + n * tensor1View.strides[2];
                        size_t tensor2Idx =
                            batch * tensor2View.strides[0] + n * tensor2View.strides[1] + k * tensor2View.strides[2];
    ...
    BaseTensor backend::CPUBackend::bmm(const BaseTensor& batch1, const BaseTensor& batch2) {
        ...
        DISPATCH_KERNEL_DTYPE_TABLE(kernelTable, bmmCpuKernel,
